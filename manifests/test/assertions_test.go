...
func TestAssertions(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:       "OB-315-DOP-100610 pass if ASPSP returns correct error",
		...
		},
		{
			...
			response: mockResponse{
				400,
				map[string]string{},
				`{"Errors":[{"ErrorCode":"UK.OBIE.Bad.Error"}]}`,
			},
			schemaSpec:                    *paymentSpecPath,
		...
		},
		{
			...
			schemaSpec:                    *paymentSpecPath,
			ExpectValidationPass:          false,
			ExpectValidationErrorContains: "HTTP Status code does not match",
		},
	}
